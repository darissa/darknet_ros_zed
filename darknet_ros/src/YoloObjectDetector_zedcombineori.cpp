...
YoloObjectDetector::YoloObjectDetector(ros::NodeHandle nh)
    : nodeHandle_(nh),
      ...
      rosBoxes_(0),
      rosBoxCounter_(0),
      imgSync_(ApproxTimePolicy(3), imagezedSubscriber_, dmapSubscriber_)
{
...
void YoloObjectDetector::init()
{
  ...
  int cameraQueueSize;
  std::string dmapTopicName; //wana++
  int dmapQueueSize; //wana++
  ...
  if (zed==true) {
    imagezedSubscriber_.subscribe(imageTransport_, cameraTopicName, cameraQueueSize);                                       
    dmapSubscriber_.subscribe(imageTransport_, dmapTopicName, dmapQueueSize);
    imgSync_.connectInput(imagezedSubscriber_, dmapSubscriber_);
